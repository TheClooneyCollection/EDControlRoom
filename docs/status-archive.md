...
- Release-prep docs now explicitly call out Windows compatibility as part of the next stable cut, and `README.md` / `docs/STATUS.md` now record early community live Windows validation while keeping macOS + CrossOver as the primary operator path.
- Release metadata is being bumped for `v1.5.0`, with README wording updated to credit macOS live validation to @NicholasClooney and community live Windows validation.
