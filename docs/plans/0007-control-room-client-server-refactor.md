...
- the checked-in Control Room message schema now matches the current websocket command and event surface instead of the pre-remote draft
- the server now exposes the message schema directly and allows cross-origin HTTP discovery so future browser clients can fetch capabilities and schema metadata without same-origin coupling
