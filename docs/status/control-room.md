...
- Status, haul, market, and trade-route picker rendering now pass through explicit ViewModels; market/trade-route interactions use display-neutral ViewActions behind Textual adapters, and `serve` now exposes the first browser Haul surface at `/haul` with authenticated `/api/haul/search` and `/api/haul/start` endpoints.
