...
- Observer transport now has shared-token auth plus `control_room connect <host>:<port>` over authenticated `GET /capabilities`/`GET /snapshot` and `WS /session`, with explicit capability metadata, a served JSON schema, permissive cross-origin HTTP discovery for future browser clients, live snapshots, activity and announcement streams, client-local TTS replay, bidirectional command envelopes, correlated errors, and remote `Ctrl-C` prompt/routine interruption.
...
- Remote validation now has `docs/operators/control-room-remote.md` plus both CLI and browser probes for HTTP and websocket smoke checks without launching the full TUI, and `serve` now hosts the browser probe directly for same-origin web-client validation.
