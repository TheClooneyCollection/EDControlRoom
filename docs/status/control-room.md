...
- Status, haul, market, and trade-route picker rendering now pass through explicit ViewModels; market/trade-route interactions use display-neutral ViewActions behind Textual adapters, and `/haul` uses authenticated all-operator websocket hydrate/search/select/start/stop/pause/resume plus server-retained selected/running trade-route hydrate, instant-mode toggle over `/session`, responsive desktop/mobile layout, live routine/activity panels, distinct home/current system values, explicit `after_run`/`now` stop modes, and capped reconnect recovery.
