...
- Status, haul, market, and trade-route picker rendering now pass through explicit ViewModels; market/trade-route interactions use display-neutral ViewActions behind Textual adapters, and `/haul` uses websocket hydrate/search/start/stop plus instant-mode toggle over `/session` with live routine/activity panels and explicit `after_run` versus `now` stop modes.
