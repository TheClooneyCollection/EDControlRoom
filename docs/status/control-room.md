# Control Room Status
## Current
- Plan 0008 is the active Control Room architecture target: one local-first `ControlRoomApp` composed with data sources, view models, view actions, and execution dependencies; local and remote execution/data-source dependencies exist, app dispatch helpers route through execution, status/haul/market panels plus connect runtime/history/activity/role/ship context read through data-source-backed paths, market presentation commands route through view actions, and `serve` exposes authenticated no-snapshot HTTP/websocket hydrate messages on initial load and live data changes.
- Snapshot is removed from the remote wire protocol: `serve` no longer advertises or serves `/snapshot`, websocket `state.snapshot`, or `command.request_snapshot`, and the remote message parser no longer accepts snapshot messages; remaining snapshot code is internal/local compatibility pending removal.
