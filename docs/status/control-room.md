...
- `connect` now bootstraps from `/hydrate`, derives only the temporary legacy view state needed by the current app constructor, and no longer requests snapshots on startup/reconnect; remaining snapshot code is server/client compatibility surface pending removal.
