# Control Room Status
## Current
- Control Room now supports `home` to route to `control_room.home_system`, plus `home set <system>` to write that setting into config; if the app started from the default example-config fallback, the setter creates repo-root `config.toml` instead of editing `config.example.toml`.
