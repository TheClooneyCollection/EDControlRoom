...
- Remove remaining `_sync_view_snapshot()` calls from read-only panel refresh paths, add websocket data update handling, then remove the old `/snapshot`-driven connect path.
