...
- `serve`/`connect` now support a real active-operator role assignment path, but deeper live validation is still needed for routine-heavy remote execution and replay flows under the headless host.
