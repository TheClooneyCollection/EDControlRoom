...
- `ControlRoomApp` no longer stores/applies backend snapshots; UI panels read via data-source/view-model paths, and remaining snapshot helpers are confined to backend/server/protocol compatibility.
...
## Next
- Continue plan 0008 by removing remaining internal snapshot compatibility from the base app/backend now that connect no longer owns a separate app subclass.
