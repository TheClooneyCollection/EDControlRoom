...
- Observer transport now has shared-token auth plus `control_room connect <host>:<port>` over authenticated `GET /capabilities`/`GET /snapshot` and `WS /session`, with explicit capability metadata for future clients, live snapshots, activity and announcement streams, client-local TTS replay, bidirectional command envelopes, correlated errors, and remote `Ctrl-C` prompt/routine interruption.
