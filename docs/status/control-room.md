...
- Status, haul, market, and trade-route picker rendering now pass through explicit ViewModels; market/trade-route interactions use display-neutral ViewActions behind Textual adapters, and `/haul` uses websocket hydrate/search/start over `/session` with live routine and activity panels.
