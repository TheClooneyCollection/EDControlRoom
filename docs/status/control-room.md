...
- Wire `serve` to emit `control_room.hydrate` from `ControlRoomDependencies.data_source`, then build remote data sources that consume hydrate/update messages directly instead of snapshots.
