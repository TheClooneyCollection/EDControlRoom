...
- Status, haul, market, and trade-route picker rendering now pass through explicit ViewModels; market/trade-route interactions use display-neutral ViewActions behind Textual adapters, and `/haul` uses authenticated all-operator websocket hydrate/search/start/stop plus instant-mode toggle over `/session` with responsive desktop/mobile layout, no-restart HTML refresh on browser reload, live routine/activity panels including scrollable full activity history, distinct hydrated home/current system values, explicit `after_run`/`now` stop modes, and visible connection recovery with capped exponential reconnect.
